...
//La partie de recupe de l'id et script pour modifier
    ...
    if($_SERVER["REQUEST_METHOD"]=='GET'){
        ...
        if(isset($_POST['modifierReception'])){
            ...
            $sql = "UPDATE `receptionpoussin` SET `poidspese` = '$poidspese', `etatbobine` = '$etatbobine', `epaisseur` = '$epaisseur',`lieutransfert` = '$lieutransfert',`poidsdeclare` = '$poidsdeclare'
            , `nbbobine` = '$nbbobine', `nbbobineactuel` = '$nbbobine', `couleurhistorique` = '2' WHERE `idreceptionpoussin` = ?;";
            //$result = $db->query($sql); 
            $sth = $db->prepare($sql);    
