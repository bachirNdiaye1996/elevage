<?php
    ...
    if(isset($_POST['CreerUtilisateur'])){
        ...
        if(!empty($_POST['username']) && !empty($_POST['email']) && !empty($_POST['nomcomplet']) && !empty($_POST['niveau'])){
            $insertUser=$db->prepare("INSERT INTO `utilisateur` (`id`, `username`, `password`, `email`, `nomcomplet`, `datecreation`, `actif`)
            VALUES (NULL, ?, ?, ?, ?, current_timestamp(), '1');");
            $insertUser->execute(array($username,$password,$email,$nomcomplet));
